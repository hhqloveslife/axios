...
class HQRequest {
  ...
  request<T = any>(config: HQRequestConfig): Promise<T> {
  ...
  get<T>(config: HQRequestConfig): Promise<T> {
    return this.request<T>({ ...config, method: "GET" });
  }

  post<T>(config: HQRequestConfig): Promise<T> {
    return this.request<T>({ ...config, method: "POST" });
  }

  delete<T>(config: HQRequestConfig): Promise<T> {
    return this.request<T>({ ...config, method: "DELETE" });
  }

  patch<T>(config: HQRequestConfig): Promise<T> {
